    ...
    try:
        ...
        try:
            ...
            strategy_result = {
                ...
                "strategy_summary": (
                    "Strategist Agent could not generate a strategy, so the fallback is further research only."
                ),
                "position_guidance": "Use this result for research only. Do not make aggressive decisions from this output.",
                "leverage_guidance": "Do not use leverage in this prototype.",
                "watchlist_status": "Research only",
                "conditions_to_reconsider": [
                    "Rerun the pipeline after checking Strategist Agent code."
                ],
                "risk_note": str(strategy_error),
        ...
        # -----------------------------
        # 10. Groq LLM Recommendation / Report Agent
        # -----------------------------
        ...
        # -----------------------------
        try:
            with st.spinner("Storage Agent is saving this pipeline run to SQLite memory..."):
            ...
            storage_result = {
                "success": False,
                "summary": f"Storage Agent could not save this run: {storage_error}",
            }
        ...
        st.write(f"**Storage Agent:** {storage_result.get('summary', 'N/A')}")
        ...
        if llm_single_stock_report.get("llm_available"):
            ...
            st.warning(
                ...
                "or skipped LLM output. Check GROQ_API_KEY and groq."
            )
        ...
        st.header("11. Storage Agent Output")
...
st.info(
    ...
    "The Groq Report Agent will simplify it into key points, risks, and possible impact."
)
...
if st.button("Simplify Financial Text"):
    ...
    if llm_report_agent is None:
        ...
        with st.spinner("Groq Report Agent is simplifying the text..."):
        ...
        if simplification_result.get("llm_available"):
            ...
            st.warning(
                "Groq API was not available. Please check GROQ_API_KEY and groq installation."
            )
...
# -----------------------------
st.divider()
st.header("SQLite Storage Dashboard")
st.caption("Local persistent memory for pipeline runs, agent outputs, paper decisions, rewards, training metadata, and reports.")

try:
    dashboard_storage_agent = StorageAgent()

    if st.button("Refresh Storage Summary"):
        storage_summary = dashboard_storage_agent.get_storage_summary()
        st.json(storage_summary)

    with st.expander("Show recent stored pipeline runs", expanded=False):
        recent_runs = dashboard_storage_agent.get_recent_pipeline_runs(limit=10)
        if recent_runs:
            st.dataframe(pd.DataFrame(recent_runs), use_container_width=True, hide_index=True)
        else:
            st.info("No stored pipeline runs yet.")
except Exception as storage_dashboard_error:
    st.warning(f"Storage dashboard is unavailable: {storage_dashboard_error}")
